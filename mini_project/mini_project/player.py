...
class Player:
    def __init__(self, screen_width: float, screen_height: float, image_path: str):
        ...
        self.player_image: Surface = scale(self.image, (self.width, self.width))
        self.hit_box: Rect = self.get_hit_box(x=self.x, y=self.y)
        
    def get_hit_box(self, x: int, y: int) -> Rect:
        return self.player_image.get_rect(center=(x, y))
    ...
    def reset(self,screen_width: float, screen_height: float):
        ...
        self.move_speed = 5
        self.player_image: Surface = scale(self.image, (self.width, self.width))
        self.hit_box: Rect = self.get_hit_box(x=screen_width // 2, y=screen_height // 2)
    ...
    def draw(self, screen: Surface):
        screen.blit(self.player_image, self.hit_box.topleft)

    def try_move(self, max_width, max_height) -> bool: 
    ...
    def grow(self):
        ...
        if self.width < 100:
            self.width += 1
            self.score += 1
            self.player_image = scale(self.image, (self.width, self.width))
